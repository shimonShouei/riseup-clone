 ...
 * SECURITY.md R6 teardown contract can be unit-tested with fakes — no Chrome, no
 * network, any Node version.
 */
 ...
 *
 * SECURITY.md R5/R6: we launch a browser we own, run exactly one scrape, and
 * ALWAYS close it in `finally` — even when the scrape throws — so no Chrome
 * profile, cache, cookie jar, or bank session lingers on disk or in memory.
 * Credentials are passed straight through as function arguments and never
 * stored, logged, or written anywhere.
 */
